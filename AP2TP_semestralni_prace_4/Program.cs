...
namespace AP2TP_semestralni_prace_4
{ 
    /*                                                                                                
    ...
    internal static class Program
    {
        public static void Main(string[] args)
        {
            ...
            for(var i = 1; i < symbolCount; i++)
            {
                ...
                var firstPartCount = 0;
                var secondPartCount = 0;

                foreach (var symbol in firstPart)
                {
                    firstPartCount += symbol.ReturnCount();
                    Console.WriteLine($"SYMBOL: {symbol.ReturnText()}");
                    Console.WriteLine($"COUNT: {firstPartCount}");
                }
            ...
            /* Methods here */
            void ReturnSymbolInfo(Symbol symbol) =>
                Console.WriteLine( $"Symbol {symbol.ReturnText()} -> " +
